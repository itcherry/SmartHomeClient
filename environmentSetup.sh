...
function copyEnvVarsToGradleProperties {
    ...
    if [ ! -f "$GRADLE_PROPERTIES" ]; then
        ...
        echo "FABRIC_TOKEN=$FABRIC_TOKEN" >> $GRADLE_PROPERTIES
        echo "GOOGLE_API_JSON=$GOOGLE_API_JSON" >> $GRADLE_PROPERTIES
        echo "TELEGRAM_CHAT_ID=$TELEGRAM_CHAT_ID" >> $GRADLE_PROPERTIES
        echo "TELEGRAM_TOKEN=$TELEGRAM_TOKEN" >> $GRADLE_PROPERTIES
    fi
}